...
import { Sparkles, ZoomIn, ZoomOut, Layers, SlidersHorizontal } from 'lucide-react';
  ...
  return (
    <div className="relative w-full rounded-[2rem] overflow-hidden bg-[#F0EBE5]/40 border border-[#DED8CF] shadow-soft select-none">
      ...
      <div className="px-4 py-2 bg-slate-900/60 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
        ...
        <span className="font-mono text-slate-500">Split: {Math.round(sliderPosition)}% / {100 - Math.round(sliderPosition)}%</span>
      </div>
    </div>
  );
};
